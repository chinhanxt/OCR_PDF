...
class DOCXBuilder:
    ...
    def build_docx_for_gemini(self, pages_metadata: list, output_docx_path: str, model_name: str = "Gemini 2.5 AI"):
        doc = docx.Document()
        self._set_document_styles(doc)

        title = doc.add_paragraph()
        title_run = title.add_run(f"KẾT QUẢ BÓC TÁCH DỮ LIỆU ({model_name.upper()})")
        title_run.bold = True
        title_run.font.size = Pt(18)
        title_run.font.color.rgb = RGBColor(14, 116, 144) # Cyan / Teal
        title.alignment = WD_ALIGN_PARAGRAPH.CENTER

        sub = doc.add_paragraph()
        sub_run = sub.add_run(f"Tổng số trang: {len(pages_metadata)} | Google Multimodal Vision AI Model")
        sub_run.font.size = Pt(10)
        sub_run.font.italic = True
        sub_run.font.color.rgb = RGBColor(100, 116, 139)
        sub.alignment = WD_ALIGN_PARAGRAPH.CENTER
                ...
                if max_row > 0 and max_col > 0:
                    ...
                    for c in table_cells:
                        ...
                        if r_idx < max_row and c_idx < max_col:
                            ...
                            cell.text = c["text"]
                            if r_idx == 0:
                                shading_elm = parse_xml(r'<w:shd {} w:fill="E0F2FE"/>'.format(nsdecls('w')))
                                cell._tc.get_or_add_tcPr().append(shading_elm)
                                for p in cell.paragraphs:
                                    for r in p.runs:
                                        r.bold = True
        ...
        doc.save(output_docx_path)
        print(f"✅ Gemini DOCX created at: {output_docx_path}")
